...
gates `/api/*` via a signed session cookie. Inference is **not** proxied — the
web UI calls the configured OpenAI-compatible provider (e.g. a LAN llama.cpp
server) directly from the browser, so that server must allow CORS from the web
UI origin (e.g. start llama.cpp with `--cors '*'`).
