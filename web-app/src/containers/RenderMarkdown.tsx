 ...
 * (the full `normalizeLatex` does so via its masking pipeline). Used for the
 * streaming path, where the full masking pass + cache is too costly per chunk.
 */
...
function RenderMarkdownComponent({
  ...
  // Full `normalizeLatex` runs the masking pipeline + cache lookup; during
  // streaming the cache never hits and that pass is a major cause of UI freezes.
  // Run only the cheap delimiter transforms (no masking, no caching) so `\(...\)`
  // still converts to inline math and `$5` stays literal, while keeping each
  // batch O(n). The final (non-streaming) render re-runs the full protective
  // normalizeLatex, which also restores code-block safety.
  const normalizedContent = useMemo(
    () =>
      isStreaming
        ? normalizeMathDelimiters(effectiveContent)
        : normalizeLatex(effectiveContent),
