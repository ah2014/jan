...
describe('ThreadDetail route', () => {
  ...
  it('edit forks a new version that preserves kept image attachments', () => {
    ...
    // A new sibling version is added; its content carries the kept image.
    expect(h.messagesState.addMessage).toHaveBeenCalled()
    const added = h.messagesState.addMessage.mock.calls.map((c: any[]) => c[0])
    const sibling = added.find((m: any) =>
      m.content?.some(
        (c: any) => c.type === 'text' && c.text?.value === 'edited text'
      )
    )
    expect(sibling).toBeTruthy()
    const content = sibling.content as any[]
    ...
    // Editing a user message regenerates the new branch.
    expect(h.mockRegenerate).toHaveBeenCalledWith(
      expect.objectContaining({ messageId: expect.any(String) })
    )
  })

  it('edit fork drops image attachments not in keptImages', () => {
    ...
    expect(
      content.some(
        (c) => c.type === 'image_url' && c.image_url?.url === keptUrl
      )
    ).toBe(true)
    expect(
      content.some(
        (c) => c.type === 'image_url' && c.image_url?.url === droppedUrl
      )
    ).toBe(false)
  })

  it('edit on an assistant message forks a version preserving kept images without regenerating', () => {
    const imageUrl = 'data:image/png;base64,IMG1'
    h.messagesState.getMessages = vi.fn(() => [
      {
        id: 'u1',
    ...
    // Assistant edits fork a version but do not regenerate.
    expect(h.mockRegenerate).not.toHaveBeenCalled()
  })
